...
| 5 | a death read as success or failure; a mutator still running when the next one starts | no `result` means unknown and a fresh read; normal completion only by the supervising core, after the child exits, no worker is still present (the controllers remaining as expected) and the postcondition holds; lost supervision is a barrier for the rest of the boot, in both interfaces, then reconciled into no effect, the expected effect, or something unexpected that stays blocked | a descendant that leaves the group is not seen by inspection: the child registry admits as managed mutators only programs that leave nothing running, or that name an owner and completion check | `sup-completion-controllers-live`, `sup-completion-worker-lingers`, `sup-identity-unknown`, `sup-core-death-mutator-live`, `sup-mutator-escaped-pgid`, `sup-mutating-daemon-classification`, `sup-unsupervised-blocks`, `sup-boot-clears`, `sup-post-reboot-reconcile`, `sup-post-reboot-unexpected`, `sup-no-reclaim-pid`, `sup-read-orphan-no-barrier`, `sup-reader-death` |
...
| 55 | diagnostics filling the temporary filesystem, or their loss changing an outcome | a reviewed registry classes each child; a read child drains through `tail -c`, never holding or writing more than 65 281 bytes; hard limits over every retained byte, headers and fixed-size summaries included (65 536 a child, 262 144 a request, 4 194 304 a session), reserved before appending; a saturated scope drains to `/dev/null` and adds nothing; a mutating child's output goes to `/dev/null` or its functional destination, and one that needs a terminal is a handoff; a capture failure changes no outcome | the scratch is not a security boundary, and removal is not erasure | `diag-*` |
| 56 | a session's scratch removed under a live process, or never removed | controllers and workers recorded by PID, start time and boot session, an unknown identity counting as alive; the live launcher cleans up its own scratch once its frontend, cores and workers are gone and no operation names it; a later launcher reclaims only when every recorded identity is dead, no process names the folder and no operation names it | — | `sup-owner-cleanup`, `sup-owner-cleanup-refused`, `sup-reclaim-live-controller`, `sup-reclaim-live-core`, `sup-reclaim-live-worker`, `sup-reclaim-operation-barrier`, `sup-reclaim-quiescent`, `sup-frontend-death-core-live`, `sup-pid-reuse`, `sup-identity-unknown` |
